...
def single_model_reviewer_slots_setting(
    ...
    *,
    ...
    exact measured routed-model identity.  Advisory uses the Claude SDK namespace,
    so it cannot honestly be populated from a provider-routed benchmark model and is
    disabled instead.  The runtime's strict parser validates the generated bytes;
    this helper does not maintain a second reviewer schema.
    """
    target = str(model or "").strip()
    if not target:
        ...
        try:
            ...
            if reviewer_config.advisory.enabled:
                mismatches.append(
                    ...
                    "disable the Claude SDK advisory rather than putting a routed model id "
                    "into its model namespace"
                )
...
def pin_single_model(
    ...
    *,
    ...
    # The structured panel disables advisory explicitly. A provider-routed model
    # id is not a valid Claude SDK model spelling, so erase any inherited value.
    sink["CLAUDE_CODE_MODEL"] = ""
    if review_effort:
