...
| ABI-2 | plan-item | task-result `_schema_version=1` per Q8=B safe-B: no legacy converter, quarantine on unstamped/future/malformed/retired-`until_deadline` rows with log-only visibility (owner 6.3=B: one batched durable event, no UI counter, no chat notice); state.json/queue_snapshot get a stamp on write, shape unchanged | retain | done | F3 | tests/test_task_result_schema_quarantine.py — F12 semantics: future-refusal, malformed, idempotency, N−1, rollback |
