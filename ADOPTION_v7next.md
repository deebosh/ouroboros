...
| ABI-8 | plan-item | Handler-ABI finale: tool handlers return ToolResult, not str (the true D02 finale) — pulled into v7.0 by Q16=A («7.1» не существует) | retain | pending | F3 | tests/test_core_native_results.py + tests/test_control_native_results.py extended to handler signatures |
