...
from ouroboros.consciousness_context import (  # noqa: F401 - re-exported for tests/consumers
    _ConsciousnessOverflow,
    _label_section,
    bounded_identity_for_low_mode,
    graceful_assemble,
    slim_drive_state,
)
...
class BackgroundConsciousness:
    ...
    def _build_context(
        ...
        *,
        ...
        dropped first when the assembled context overflows ``BG_CONTEXT_MAX_CHARS``).
        P1 cognitive artifacts (BIBLE.md, identity, scratchpad, knowledge index,
        Pattern Register, dialogue horizon, ARCHITECTURE nav-map, governance,
        identity-completeness directive) carry drop_priority=0 and are never
        dropped. Non-P1 sections (backlog digest, observations, runtime, drive
        state, recent reflections, chat tail) drop in a fixed order so an
        overflowing wakeup degrades to a runnable context instead of skipping
        entirely (structural fix for ``ibl-local-31f19191be34``).

        Mode-aware assembly honours ``OUROBOROS_CONTEXT_MODE``: in ``low`` we
        skip the improvement backlog digest and the ephemeral observations
        queue, bound identity, and slim/bound drive state. ARCHITECTURE is
        forced to nav-map in BOTH modes — the consciousness loop only navigates
        the doc, never reads the whole body.
        """
        ...
        # Drop-priority constants (lower = dropped first on overflow). P1
        # cognitive artifacts carry P1=0 and are never dropped by degradation.
        P1 = 0
        DROP_FIRST = 10   # backlog digest, observations, recent reflections/events
        ...
        # BIBLE.md full (P1); ARCHITECTURE forced to nav-map regardless of mode
        # (ibl-local-31f19191be34: the loop never pays the full-text cost).
        ...
        mem_sections = build_memory_sections(
        ...
        for idx, m in enumerate(mem_sections):
            ...
            if (
                ...
                bounded = bounded_identity_for_low_mode(m)
                if bounded != m:
                    m = bounded
                    label = "identity_bounded_low_mode"
                    bounded_identity_done = True
            parts.append(m)
            sections.append((label, len(m), P1))
        ...
        # Improvement backlog digest: low-mode skip (action-hint, not a core
        # cognitive artifact). DROP_FIRST in max.
        if context_mode == "low":
            ...
            try:
                ...
                try:
                    ...
                    if path.exists():
                        ...
                        sections.append(("backlog_unavailable", len(unavailable), P1))
                except Exception:
                    pass
        ...
        # Slim drive-state projection (ibl-local-31f19191be34): strips the
        # 453K-char usage_accounting.by_root map in both modes; further bounds
        # to ~30K in low mode.
        drive_state = slim_drive_state(self._repo_dir, self._drive_root, context_mode=context_mode)
        if drive_state:
        ...
        recent_sections = build_recent_sections(
            memory, env, task_id="", chat_coverage_out=recent_chat_coverage,
        )
        for idx, r in enumerate(recent_sections):
            priority = DROP_LATE if r.startswith("## Recent chat") else DROP_FIRST
            parts.append(r)
        ...
        # Observations: low-mode skip (ephemeral queue-injected hints, not
        # memory; deferring to the next wakeup is safe). The snapshot/state
        # read above still runs in low mode so gaps stay tracked.
        ...
        if self._identity_unresolved_sources:
            ...
            sections.append(("identity_update_completeness", len(completeness), P1))

        if include_observations and observation_rendered:
            parts.append(observation_rendered)
            sections.append(("observations", len(observation_rendered), DROP_FIRST))
        else:
            sections.append(
        ...
        # Replaces the prior all-or-nothing skip (ibl-local-31f19191be34).
        _BG_TOTAL_WARN_CHARS = BG_CONTEXT_WARN_CHARS
        _BG_TOTAL_MAX_CHARS = BG_CONTEXT_MAX_CHARS
        full_text, total_chars, dropped = graceful_assemble(
