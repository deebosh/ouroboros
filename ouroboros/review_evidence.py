...
def collect_review_evidence(
    ...
    *,
    ...
    # ibl-00615dbd1a16: a different task on the shared repo tree that recorded its
    # own stale marker must NOT show up in this task's `current_repo` panel. An
    # empty stored task_id (legacy/unattributed) still matches any caller; a
    # non-empty stored task_id only matches itself or an unset task_id query.
    stale_matches_task = (not task_id) or str(getattr(state, "last_stale_task_id", "") or "") in ("", task_id)
    _stale_visible = stale_matches_repo and stale_matches_task
    ...
    evidence = {
        ...
        "current_repo": {
            ...
            "stale_reason": str(getattr(state, "last_stale_reason", "") or "") if _stale_visible else "",
            "stale_ts": str(getattr(state, "last_stale_from_edit_ts", "") or "") if _stale_visible else "",
        },
