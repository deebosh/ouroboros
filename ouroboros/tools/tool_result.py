...
TOOL_CODE_SPECS: Mapping[str, ToolCodeSpec] = MappingProxyType(
    {
        ...
        # ibl-a0348d742b9b: a non-semantic pre-flight root hint. The dispatch
        # SKIPPED the call because the requested root+operation was matrix-denied
        # and an allowed alternative exists — the gate never ran, so ``status``
        # stays ``ok`` (not a failure), but the outcome bucket is its own typed
        # ``preflight_root_hint`` so the classifier / reflection scan / web
        # client can tell a redirected call from a plain success.
        "PREFLIGHT_ROOT_HINT": _code_spec(
            "ok",
            "preflight_root_hint",
            "warning",
            "re-issue with the suggested root named in the result text",
        ),
...
_EXACT_IDENTIFIER_CODES = MappingProxyType(
    {
        ...
        "PREFLIGHT_ROOT_HINT": "PREFLIGHT_ROOT_HINT",
        ...
        # Fork bump_version: a malformed version string is an argument error, not
        # a bare warning (the "_INVALID" suffix has no family rule).
        "BUMP_VERSION_INVALID": "TOOL_ARG_ERROR",
