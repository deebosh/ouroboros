...
def _repo_write(ctx: ToolContext, path: str = "", content: str = "",
    ...
    # leading segments unmistakably spell an absolute runtime/home path would
    # silently join against the repo base and land in the wrong tree. Catch the
    # obvious shapes BEFORE the binding/resolve step. Absolute paths (leading
    # "/") are already handled by the boundary guard.
    for e in write_list:
        reason = _git()._misrelativized_runtime_path_reason(e["path"])
        if reason:
            return (
