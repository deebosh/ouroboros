...
# ibl-b1f10cece0eb: the two advisory-block shapes an inline advisory_review can
# actually clear — no run matches the current snapshot, or the last one could
# not be parsed. A SyntaxError preflight block or an "advisory current but
# obligations still open" block reproduces identically on a re-run, so those are
...
def _advisory_and_tests_gate(
    ...
    *,
    ...
    if (
        ...
        # ibl-b1f10cece0eb: a missing / edit-staled advisory is the single most
        # common commit_reviewed bounce — the task edited the very files it was
        # sent to edit, which invalidates any advisory it ran earlier, so a
        # production-touching change lands here on the first commit_reviewed and
        # has to leave, call advisory_review by hand, and come back (172-199
        # rounds observed on such tasks vs 45 for a tests-only change that never
        # needs a bump). Run the SAME advisory the agent would run, inline,
        # against the current snapshot, then re-check once. A real
        # AdvisoryRunRecord is written (not a bypass), so the compensating
        # test-preflight coupling below is unchanged. One attempt only: a
        # genuine block (SyntaxError preflight, open obligations, a critical
        # advisory finding) survives the re-check and is returned as before.
        try:
            ctx.emit_progress_fn(
                "No fresh advisory for this snapshot — running advisory_review inline "
...
def _restore_to_head(ctx: ToolContext, confirm: bool = False,
    ...
    if normalized_paths:
        ...
        # restore cleaned (the helper recomputes it from epoch evidence as a
        # sanity check).
        ...
        return _vcs_result(
            f"Restored {len(normalized_paths)} path(s) to HEAD." + progress_note,
            binding,
        )
    else:
        prior_status = _git_status_porcelain(repo_dir)
