...
# 878a0747 (ibl-local-2d41a76a0c44): opt-in advisory inline per-file cap
# (~15k tokens). A single large generated carrier (uv.lock, vendor dumps, install
# pages, big fixtures) below _FILE_SIZE_LIMIT can still blow the ~1.6M-char
# aggregate advisory budget beside the governance docs + diff + checklist.
_ADVISORY_INLINE_FILE_LIMIT = 60_000
...
def build_touched_file_pack(
    ...
    *,
    ...
    ``inline_file_limit`` (opt-in, advisory only — 878a0747): when set, a single
    file whose content exceeds it is omitted from the inline pack (disclosed via
    the returned list) instead of inlined in full; the staged diff hunk still
    carries the change. Scope/triad callers leave it ``None`` so their own fit
    ladders keep ownership of degradation (ibl-local-2d41a76a0c44)."""
    if paths is None:
    ...
    for rel in paths:
        ...
        # Advisory inline-budget cap (opt-in via inline_file_limit, 878a0747):
        # omit an oversized-but-sub-1MB carrier from the inline pack, disclose
        # via `omitted`; the staged diff hunk still carries the actual change.
