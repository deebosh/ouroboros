...
def derive_loop_outcome(
    ...
    *,
    ...
    The two phases (classify execution health, then finalize the
    review/objective axes and assemble the outcome dict) live in the leaf
    module ``ouroboros._outcome_axes`` (ibl-500e3d3b40f2 / ibl-6710dd209229 —
    split purely to shrink this module and this function under the
    size-ratchet caps; no behavior change). The import is function-local on
    both sides of the module boundary to avoid a load-time import cycle
    between ``outcomes.py`` and ``_outcome_axes.py``.
    """
    from ouroboros._outcome_axes import _classify_loop_execution_status, _finalize_objective_axes

    classification = _classify_loop_execution_status(
        final_text, usage, llm_trace, repo_dir=repo_dir, attributed_paths=attributed_paths,
    )
    outcome = _finalize_objective_axes(classification, llm_trace, usage)
