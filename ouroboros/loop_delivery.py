...
def _resolve_delivery_control(
    ...
    if _loop()._is_substantive_final_prose(raw, candidate.full_text) and not is_control_intent:
        ctx.messages.append({"role": "assistant", "content": raw})
        tools._ctx._delivery_control_required = False
        updated = _loop()._replace_delivery_candidate(
...
def _no_tool_final_answer(
    ...
    if control_state == "fresh" and str(content or "").strip():
        # ibl-d29bc3cc9d67: a fresh prose turn must not latch a terminal
        # candidate while this task's own tracked edits are still uncommitted —
        # re-loop so commit_reviewed stays reachable (bounded nudge budget).
        if _loop()._withhold_prose_for_uncommitted_work(
