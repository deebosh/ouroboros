...
BAND_PATHS = {
    ...
    "ouroboros/acceptance_dialogue.py": "Entered the band from 853 lines by absorbing the acceptance packet builder and panel execution WHOLE from loop.py (which sits at its shrink-only byte debt): the acceptance surface's one owner now holds contract, obligations, dialogue, paid identity, packet and panel, so the retrieving work order (three deliveries) lands beside the packet it renders instead of in a fourth module.",
    ...
    "ouroboros/delegate_supervision.py": "Entered the band (1000 -> 1006 lines) with the absent-ledger fast-path precision of the settled-spend coordination fact (owner R55 batch, reviewer major 3): known-zero only on an explicit FileNotFoundError from os.stat, every other stat outcome through the canonical usage reader. The fact belongs beside the sibling coordination facts it is polled with; no helper or neighbour module was minted for six lines.",
    ...
    "tests/test_external_review_script.py": "Contract tests of the operator review script stay one file with the script they pin (preflight, substrate path set, verdict delegation, receipts); the substrate set grows with the substrate.",
