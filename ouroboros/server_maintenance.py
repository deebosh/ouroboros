...
def _periodic_store_gc() -> None:
    """Every ~6h: keep the task-results store bounded without a restart
    (razzant/ouroboros#139). The startup sweep does the same on boot; a
    long-lived supervisor needs it on a cadence too.

    Usage-ledger compaction is NOT run here: upstream's CPL4-C6
    (``ouroboros/usage_compaction.py``, byte-threshold triggered on the append
    path) owns that; the fork's divergent ``compact_ledger`` was dropped in the
    v7 merge.
    """
    ...
    try:
        ...
        if task_results_report.get("pruned") or task_results_report.get("errors"):
...
def _periodic_supervisor_maintenance(last_custody_reap: list, last_review_reconcile: list) -> None:
    ...
    (every 300s) + store GC / stale review-continuation sweep (every ~6h). Each
    cadence gates itself via its own last-run marker."""
    ...
    # razzant/ouroboros#139: a long-lived supervisor bounds the append/result
    # stores on a ~6h cadence too (the startup sweep only covers boot). Each
    # gates itself via its own last-run marker.
    _periodic_store_gc()
