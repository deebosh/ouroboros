...
from ouroboros.llm_google_genai import GoogleGenAIChatMixin
...
class LLMClient(GoogleGenAIChatMixin):
    ...
    def _parameter_rejection_error(exc: BaseException) -> bool:
        ...
        return any(
            ...
            for marker in (
                ...
                "extraneous",
                "not permitted",
                # VALUE-rejection families (v6.73.2). Mandatory-enable family — the parameter is
                # supported but its DISABLED/bottom value is forbidden (e.g. Gemini "Reasoning is
                # mandatory for this endpoint and cannot be disabled"); routed to the effort-FLOOR
                # branch by _mandatory_value_rejection (same _MANDATORY_VALUE_MARKERS constant).
    ...
    def _qualified_model_name(provider: str, resolved_model: str) -> str:
        ...
        if provider == "google_genai":
            ...
            # so cross-provider aggregation, pricing, and per-token cost
            # projections in pricing.py treat it the same way as the OpenRouter
            # id form. Anything that reads ``provider=google_genai`` already
            # short-circuits to the native HTTP path; the qualified name is
            # only for ledger/aggregation surfaces.
    ...
    def _resolve_remote_target(
        ...
        if provider == "google_genai":
            # v6.103.8 — direct Google Generative AI route. NOT OpenAI-compatible:
            # ``/v1beta/models/{model}:generateContent`` with key as a query param
            # (or ``x-goog-api-key`` header — same endpoint, no SDK swap). The base
            # URL is persistable (OUROBOROS_GEMINI_BASE_URL); the key is env-only by
            # design (provider_models.PROVIDER_CREDENTIAL_GROUPS["google_genai"]) and
            # has NO SETTINGS_DEFAULTS row, so an empty/missing env var lands here as
            # ``api_key=""`` and ``provider_has_credentials`` reports False — the
            # owner sees a clean missing-credential error rather than a 401.
            gemini_base_url = (
                os.environ.get("OUROBOROS_GEMINI_BASE_URL", "") or ""
            ).strip() or "https://generativelanguage.googleapis.com"
            return {
                "provider": provider,
                "resolved_model": resolved_model,
                "usage_model": usage_model,
                "api_key": os.environ.get("OUROBOROS_GEMINI_API_KEY", "") or "",
                "base_url": gemini_base_url,
                "default_headers": {},
                "supports_openrouter_extensions": False,
                "supports_generation_cost": False,
                # `gemini_native: True` is a single signal the chat path reads to
                # short-circuit to ``_chat_google_genai`` instead of the OpenAI
                # client — keeps the rest of the dispatch table provider-agnostic.
                "gemini_native": True,
            }
    ...
    async def chat_async(
        ...
        if target.get("provider") == "gigachat":
            ...
            result[1]["ledger_attempt_ids"] = list(attempt_ids)
            return result
        if target.get("provider") == "google_genai" or target.get("gemini_native"):
            # Same pattern as gigachat — synchronous ``requests`` call offloaded
            # to a thread so the event loop is never blocked.
            with capture_attempt_ids() as attempt_ids:
                result = await asyncio.to_thread(
                    self._chat_google_genai, target, messages, tools, reasoning_effort,
                    max_tokens, tool_choice, temperature, no_proxy, timeout,
                )
            result[1]["ledger_attempt_ids"] = list(attempt_ids)
            return result
    ...
    def _chat_remote(
        ...
        if target.get("provider") == "google_genai" or target.get("gemini_native"):
            return self._chat_google_genai(
                target, messages, tools, reasoning_effort, max_tokens, tool_choice, temperature,
                no_proxy=no_proxy,
                timeout=timeout,
            )
