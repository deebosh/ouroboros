...
def _preflight_root_hint(
    ...
    Closes ibl-a0348d742b9b (external_workspace_task calls accumulating 5-7
    policy denials before self-correction). The hint names the FIRST allowed
    root for this operation+profile BEFORE the call fires, so the gate's typed
    refusal only happens when the hint is genuinely wrong.

    **Does NOT change policy enforcement**: the gate keeps running on every
    call. If this returns None the call proceeds exactly as before; if it
    returns a typed hint string the dispatcher surfaces that as the tool's
    result (published as ``PREFLIGHT_ROOT_HINT`` — status ``ok``, a hint, not a
    failure) and skips the dispatch.

    Three narrow gates keep false-positives structurally impossible:
    1. Tool in ``_PREFLIGHT_ROOT_HINT_TOOLS`` (curated root=-sensitive surface).
    2. ``decide_tool_access(profile, root, operation)`` returns ``allow=False``
       for the actual requested root.
    3. ``_process_root_candidates`` returns at least one allowed alternative
       (otherwise the gate is the only source of truth).
    """
...
def _execute_single_tool(
    ...
    tool_ok = True
    # ibl-a0348d742b9b: non-semantic pre-flight root hint. If the policy matrix
    # denies the requested root for this profile+operation AND a known-allowed
    # alternative exists, surface the hint as the result instead of letting the
    # dispatch burn a denial+recovery round. Published as PREFLIGHT_ROOT_HINT
    # (status ``ok`` — a hint, not a failure); any call this returns None for
    # still hits the gate exactly as before.
    _root_hint = _preflight_root_hint(tools, fn_name, args)
    if _root_hint is not None:
        result = _root_hint
        tool_result = ToolResult(status="ok", code="PREFLIGHT_ROOT_HINT", text=_root_hint)
    else:
        try:
