...
from ouroboros.config import adaptive_quorum, get_chat_model, get_context_mode, get_light_model, get_review_enforcement, get_task_review_mode, resolve_effort  # noqa: F401 -- the loop module keeps its historical import surface for the L-B leaves
...
def _is_substantive_final_prose(raw_text: str, retained_text: str) -> bool:
    ...
    (ibl-local-05b94950560c). A real final answer here is: not JSON-shaped, past
    the one-liner floor, and no shorter than the answer it would replace.
    """
...
def _uncommitted_attributed_paths(
    ...
    error or a missing/absolute repo). Host-bound (non-subagent) tasks only.
    """
