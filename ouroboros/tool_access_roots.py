...
def _coerce_real_path_local(value: Any) -> "Optional[pathlib.Path]":
    """Mirror of tools/registry.py::_coerce_real_path, inlined to avoid the
    tool_access -> tools.registry import edge. Returns None for None, empty
    strings, Mock instances, and TypeError-raising values."""
...
def resource_root_path(
    ...
    *,
    ...
    if root == "active_workspace":
        # ibl-a7530ab93441: mirror active_repo_dir_for (tools/registry.py) so a
        # self_worktree subagent whose ToolContext carries workspace_root +
        # workspace_mode (but no callable active_repo_dir) resolves to its
        # worktree, not the system repo. Falls through to the direct-chat
        # project-room lens, then repo_dir.
