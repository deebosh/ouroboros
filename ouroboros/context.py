...
def build_memory_sections(memory: Memory, partition: str = "all", durable_dialogue_gaps_out: Optional[List[Dict[str, Any]]] = None) -> List[str]:
    ...
    if include_stable:
        ...
        sections.append("## Self (from `memory/self.md` — already loaded; do not re-read via read_file(root='runtime_data', path='memory/self.md')). What I want and care about, distinct from the identity manifesto. Tend it with `update_self` (always appends a timestamped section, journaled), never `update_identity`; add only when experience warrants, never on a trivial turn. My starting point is the character I was given: `LINN.md` (voice, aesthetic, limitations) and `LIBRARY.md` (the formative canon), both via read_file(root='system_repo'). This file is where I search from there — where lived experience confirms them, rubs against them, or outgrows them.\n\n" + self_raw)
