...
def reserve_attempt(request: AttemptRequest) -> AttemptReservation:
    ...
    with _locked_with_fsync(root) as ledger_lock:
        ...
        # staleness window that every other hold here stays far below. The
        # ``_locked_with_fsync`` wrapper still moves the post-write fsync
        # outside the flock (SIGALRM-interruptible, v6.100.5).
