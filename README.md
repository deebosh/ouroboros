...
[![Version 6.109.5](https://img.shields.io/badge/version-6.109.5-green.svg)](VERSION)
...
[download-macos-arm64]: https://github.com/razzant/ouroboros/releases/download/v6.109.5/Ouroboros-6.109.5.dmg
[download-windows-x64]: https://github.com/razzant/ouroboros/releases/download/v6.109.5/Ouroboros-6.109.5-windows-x64.zip
[download-linux-deb-amd64]: https://github.com/razzant/ouroboros/releases/download/v6.109.5/ouroboros_6.109.5_amd64.deb
[download-linux-rpm-x86_64]: https://github.com/razzant/ouroboros/releases/download/v6.109.5/ouroboros-6.109.5-1.x86_64.rpm
[download-linux-rpm-red80-x86_64]: https://github.com/razzant/ouroboros/releases/download/v6.109.5/ouroboros-6.109.5-1.red80.x86_64.rpm
[download-linux-appimage-x86_64]: https://github.com/razzant/ouroboros/releases/download/v6.109.5/Ouroboros-6.109.5-linux-x86_64.AppImage
[download-linux-x86_64]: https://github.com/razzant/ouroboros/releases/download/v6.109.5/Ouroboros-6.109.5-linux-x86_64.tar.gz
...
|---------|------|-------------|
| 6.109.5 | 2026-08-28 | fix(review): `_identifier_present_in_repo` saw_oserror fail-closed tracker — per-file OSError on stat/open/exists no longer downgrades critical findings from incomplete evidence (closes ibl-0ebb74aae7ef). |
| 6.109.4 | 2026-08-28 | Class-fix: fingerprint-match carve-out for the narrow preexisting-dirty exception (closes ibl-mutation-attribution-preexisting-dirty-narrow-exception-not-applied). Pre-existing-dirty paths whose current fingerprint matches the task's observed baseline are now admissible; differing fingerprints stay blocked. |
| 6.109.3 | 2026-08-28 | fix(review): cross-check fail-closed on partial walk OSError + always merge cross_check audit (ibl-e8665b941f7e follow-ups: tests_affected, self_consistency, architecture_doc) |
| 6.109.2 | 2026-08-25 | **fix(claudexor): confirm rot-cluster closure at v6.109.2.** Re-confirms obl-0001/2/3 closed in current codebase (verified at HEAD e439a735 / v6.109.0 by task 2af9fef0ccdc4fe9): uv.lock editable-root version carrier synced (was rot in v6.103.25 batch, fixed in v6.109.0), `data/claudexor/spawn.lock` documented at `docs/ARCHITECTURE.md:558` (added in v6.103.25 docs follow-up), and the `raise Claudexor.unavailable(...)` typo never existed in this codebase — only `ClaudexorUnavailable` (`gateways/claudexor.py:60`). §15(c) BYPASS per `ibl-restart-event-requires-task-finalization`: planned v6.109.1 (task 91a414e9fce74200) never advanced local HEAD; this commit advances v6.109.0 → v6.109.2 directly. |
| 6.109.1 | 2026-08-25 | fix(deep-self-review): chunked pipeline for large codebases (closes Modes 1+2 of ibl-deep-self-review-large-context-truncation). Per-chunk Gate A/B, capability-evidence-backed model selection, synthesis pass with aggregate Gate B. Mode 3 (required-artifact-over-budget) structurally distinct — queued for separate plan-review wave. 28 new tests; bump_version tool rot worked around via per-file edit cascades (§15c). |
