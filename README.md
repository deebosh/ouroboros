...
[![Version 6.115.5](https://img.shields.io/badge/version-6.115.5-green.svg)](VERSION)
...
[download-macos-arm64]: https://github.com/razzant/ouroboros/releases/download/v6.115.5/Ouroboros-6.115.5.dmg
[download-windows-x64]: https://github.com/razzant/ouroboros/releases/download/v6.115.5/Ouroboros-6.115.5-windows-x64.zip
[download-linux-deb-amd64]: https://github.com/razzant/ouroboros/releases/download/v6.115.5/ouroboros_6.115.5_amd64.deb
[download-linux-rpm-x86_64]: https://github.com/razzant/ouroboros/releases/download/v6.115.5/ouroboros-6.115.5-1.x86_64.rpm
[download-linux-rpm-red80-x86_64]: https://github.com/razzant/ouroboros/releases/download/v6.115.5/ouroboros-6.115.5-1.red80.x86_64.rpm
[download-linux-appimage-x86_64]: https://github.com/razzant/ouroboros/releases/download/v6.115.5/Ouroboros-6.115.5-linux-x86_64.AppImage
[download-linux-x86_64]: https://github.com/razzant/ouroboros/releases/download/v6.115.5/Ouroboros-6.115.5-linux-x86_64.tar.gz
...
| 6.115.4 | 2026-09-09 | test(provider_test): bump hard-coded provider counts 7→8 / 8→9 to match the v6.114.0 settings_ui.js PROVIDER_CARDS expansion; closes the browser-module CI red at upstream-merge staleness. |
| 6.115.3 | 2026-09-09 | F-rule lint gate: import List/Optional in agent_startup_checks; delete the review_helpers/loop.py git-status + delivery-control functions duplicated after their extract-to-module refactors (compat re-exports keep them importable; the review_helpers copy's --untracked-files=all divergence ported into git_status_parse); rename the field loop var shadowing dataclasses.field in usage_ledger._build_compacted_summary; restore queue.PENDING in the test_task_admission fixture teardown; drop unused test imports/locals. ruff check --select F now clean. |
| 6.115.2 | 2026-09-09 | refactor(consolidator): centralize strip_think_blocks in memory and project historical blocks |
| 6.115.1 | 2026-09-09 | fix(consolidator): forward-only consolidation-quality class-fix (cycle 57) — `_create_block_summary` prompt rewrite (hard ~2,500-char target + explicit PRESERVE/DROP lists, prohibits verbatim source markdown), producer-side text pre-truncation in `_format_entries_for_block` (drops tool output / dumps above 2,000 chars), and `_pending_bytes` byte-trigger computation in `should_consolidate` (walks segments from `last_offset`; closes ibl-f60344038572 archived-chain latch). Existing 88KB/9-block `memory/dialogue_blocks.json` stays byte-identical per BIBLE P1 historical-record preservation. |
