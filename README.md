...
|---------|------|-------------|
| 6.101.2 | 2026-08-16 | fix: extension Settings UI forms never showed saved values, so any save wiped the rest of the fields. api_extension_settings_section() now reads the skill own state-scoped settings.json and attaches it as saved_values; settings.js passes it into renderSafeField instead of a hardcoded {}. Root cause of the telegram-bridge TELEGRAM_CHAT_ID data loss reported after a disable/enable cycle. Affects every skill using register_settings_section with a form component, not only telegram-bridge. 3 new backend tests (tests/test_extensions_api.py); 312/312 existing JS tests pass unchanged. |
| 6.101.1 | 2026-08-16 | fix: run_command guard misses single-operator shell mistakes stuffed into one argv element. The v6.93.2 fix (9224e188) only caught two-or-more AND/OR operators glued into one cmd element; a lone AND, OR, bare pipe, or semicolon in a ONE-element cmd (e.g. ["ls -la and cat file.txt"]) passed through silently to a raw [Errno 2]. New check scoped to len(cmd)==1 (where an operator-looking substring can never be legitimate value content, unlike a multi-arg commit message or grep pattern) closes the gap with a threshold-1 detector that includes a bare pipe character, which the multi-arg check deliberately excludes. Structural fix for the most-recurring backlog pattern (11 nominations, ibl-5aa29f06571d through ibl-4ecff817c661, 2026-08-11 to 08-15). |
...
| 6.100.5 | 2026-08-15 | **fix: ledger fsync runs AFTER lock release — closes the SIGKILL-on-timeout class.** The cross-process flock held by `_locked` is the substrate's atomicity guarantee; `os.fsync` was being invoked while that flock was held, and `os.fsync` is a kernel-level syscall that cannot be interrupted by pytest-timeout's SIGALRM, so every test that hit the ledger under pytest turned into a SIGKILL after the lock timeout. The fix splits the ledger write into two steps: `_append_bytes_fsync` is now fsync-free (O_APPEND atomicity preserved by the kernel); the new `_locked_with_fsync` context manager acquires the flock, lets callers append under it, and then calls `_fsync_path(ledger)` AFTER releasing the lock — the signal can interrupt the fsync because the lock is already gone, and durability is unchanged because O_APPEND completed before the flush. Every `_append_rows_locked` call site in `usage_accounting.py` now wraps under `_locked_with_fsync` instead of bare `_locked`; `_quarantine_tail`'s own fsync was already outside any ledger lock and stayed there. P9 carriers bumped (VERSION, pyproject.toml, web/package.json, web/modules/api_types.js, docs/ARCHITECTURE.md, README badge, uv.lock editable root). No `timeout = 30` change here — that is v6.100.6, a separate cycle. |
| 6.100.3 | 2026-08-14 | **chore: diagnostic containment for pytest hang in test_web_search.py — explicit non-fix.** A serial-lane registration in tests/conftest.py plus a per-test timeout decorator in tests/test_web_search.py with a rot-comment explicitly stating this does NOT fix the root cause. The underlying rot is class-fix-shaped (`ibl-system-test-fragility`: `_web_search_openai_responses` + `usage_accounting` ledger locking under MagicMock in pytest context) and lives in the improvement backlog. Cumulative with v6.100.2 (MERGE_HEAD re-establish + synchronous state.current_sha write). P9 changelog cap (5 patch) honored by rolling 6.96.2 off to git tags. |
| 6.100.2 | 2026-08-14 | **fix: re-establish MERGE_HEAD safety net for user-initiated merges + synchronous state.current_sha write after commit.** Two structural changes that close the merge-commit gap observed during the v6.100.1 merge (commit 982cce8e): (1) the commit path now mirrors the managed-update re-establish for MERGE_HEAD — if a blocked review's index reset clears the live MERGE_HEAD, the next commit would silently drop the merge parent; we restore it from the pre-fingerprint parents list before the commit. (2) Right after commit_sha is computed, state.current_sha is written synchronously, mirroring the managed-update and restore paths, so the campaign supervisor sees the actual HEAD immediately instead of waiting for reconciliation. Bounded to one file (ouroboros/tools/git.py); no behavior change for non-merge commits. |
