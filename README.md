...
[![Version 6.101.3](https://img.shields.io/badge/version-6.101.3-green.svg)](VERSION)
...
|---------|------|-------------|
| 6.101.3 | 2026-08-16 | fix: prior settings-form fix (6.101.2) landed on the wrong producer. The Settings page reads sections from GET /api/extensions -> live.settings_sections (apiClient.extensions()), not from the separate per-skill settings_section route that got saved_values first. Refactored the attach logic into one shared _attach_settings_section_saved_values helper and wired it into _build_extensions_index too, so both producers of a settings-section list carry the same guarantee. New regression test on the actual UI-consumed endpoint. |
