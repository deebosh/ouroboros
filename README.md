...
[![Version 7.0.0-rc.8](https://img.shields.io/badge/version-7.0.0--rc.8-green.svg)](VERSION)
...
[download-macos-arm64]: https://github.com/razzant/ouroboros/releases/download/v7.0.0-rc.8/Ouroboros-7.0.0-rc.8.dmg
[download-windows-x64]: https://github.com/razzant/ouroboros/releases/download/v7.0.0-rc.8/Ouroboros-7.0.0-rc.8-windows-x64.zip
[download-linux-deb-amd64]: https://github.com/razzant/ouroboros/releases/download/v7.0.0-rc.8/ouroboros_7.0.0-rc.8_amd64.deb
[download-linux-rpm-x86_64]: https://github.com/razzant/ouroboros/releases/download/v7.0.0-rc.8/ouroboros-7.0.0-rc.8-1.x86_64.rpm
[download-linux-rpm-red80-x86_64]: https://github.com/razzant/ouroboros/releases/download/v7.0.0-rc.8/ouroboros-7.0.0-rc.8-1.red80.x86_64.rpm
[download-linux-appimage-x86_64]: https://github.com/razzant/ouroboros/releases/download/v7.0.0-rc.8/Ouroboros-7.0.0-rc.8-linux-x86_64.AppImage
[download-linux-x86_64]: https://github.com/razzant/ouroboros/releases/download/v7.0.0-rc.8/Ouroboros-7.0.0-rc.8-linux-x86_64.tar.gz
...
|---------|------|-------------|
| 7.0.0-rc.8 | 2026-09-04 | **Release candidate of 7.0.0 — the v7 follow-up sprint, phase F1 (bugs and gates).** Desktop onboarding completes again (the wizard's render called a deleted function; every fresh desktop install since 6.113.3 stayed on "Saving..."), with two class gates in the browser-module lane (a static free-identifier walk over every web module and a per-step render smoke of the wizard) and bounded completion waits (Claudexor snapshot read on its own shared worker, in-process settings-document lock) that answer typed 503s instead of hanging. The in-process direct-chat turn is owner-addressable: cancel/hurry/decisions reach it through one ownership reader, and "Stop now" ends it cooperatively with zero further model calls. Extension companion processes start outside the registry lock. Reviewed to convergence (triad + scope + two adversarial agents + two delta lenses). |
| 7.0.0-rc.7 | 2026-09-03 | **Release candidate of 7.0.0** — packaged pre-release for the owner's manual test (batch №13 item 14); rc.1–rc.6 never produced a full asset set (bundle step, two Windows test pins, a bash-4 builtin on the macOS build, a pin reading its own comment, a Linux-only pin without its skip). Carries every post-batch lane: Windows kernel lock tier, W4 crash-window fixes, typed process facts, mutating-delegation E2E, split compaction suite, migration projection. Carries the Windows kernel lock tier (item 1 = B) and the split compaction suite. The release bar now runs `system-e2e-mock`, the cancellation E-suite mock lane and the campaign `--release` gate on the tag path itself. |
| 7.0.0 | 2026-09-02 | **feat: ABI 7.0 — the v7 refactor lands on the live tip.** The v7next campaign transplants the frozen v7 deltas onto the current 6.114.x line with no regressions: PluginAPI 2.0 admission with a hash-bound grandfather, generation-bound extension publication and recovery (a running worker adopts a skill enabled after boot without a restart), a kernel-enforced lock and seq-preserving compaction for the monetary usage ledger, the ABI-2 task-result schema stamp with fail-closed quarantine, honest cost names, typed supervisor event taxonomy, retired flat wall-clock timeout knobs, a domain manifest with a shrink-only SCC ceiling, the release-compatibility auditor (`scripts/rc_audit.py`), 23 mock-lane system E2E scenarios, and a green 3-OS CI matrix. See docs/v7next/ for the adoption ledger and per-lane corrections. Upgrading an existing install: the first start moves task results written by earlier versions into `data/task_results/quarantine/` (nothing is deleted) and replaces retired comma-list review settings with the shipped reviewer panel — back up `data/` first, or preview the effect read-only with `python scripts/rc_audit.py <data dir>`. |
