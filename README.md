...
[![Version 6.103.8](https://img.shields.io/badge/version-6.103.8-green.svg)](VERSION)
...
| 6.103.8 | 2026-08-17 | v6.103.8 — Google Generative AI direct provider (`google_genai::` prefix, env-only `OUROBOROS_GEMINI_API_KEY`, native `generateContent` via `requests`). Routes deep self-review off OpenRouter's exhausted credit pool that burned $11 on task c7862982. Also caps `_DEEP_MAX_OUTPUT_TOKENS` at 10k (was 100k — the rot captured by `ibl-be9ba2d99b25`). Owner-authorized bypass (precedent: `dd7cf06d`). crd-0003 unchanged. |
