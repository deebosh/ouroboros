...
[![Version 7.0.0-rc.12](https://img.shields.io/badge/version-7.0.0--rc.12-green.svg)](VERSION)
...
[download-macos-arm64]: https://github.com/razzant/ouroboros/releases/download/v7.0.0-rc.12/Ouroboros-7.0.0-rc.12.dmg
[download-windows-x64]: https://github.com/razzant/ouroboros/releases/download/v7.0.0-rc.12/Ouroboros-7.0.0-rc.12-windows-x64.zip
[download-linux-deb-amd64]: https://github.com/razzant/ouroboros/releases/download/v7.0.0-rc.12/ouroboros_7.0.0-rc.12_amd64.deb
[download-linux-rpm-x86_64]: https://github.com/razzant/ouroboros/releases/download/v7.0.0-rc.12/ouroboros-7.0.0-rc.12-1.x86_64.rpm
[download-linux-rpm-red80-x86_64]: https://github.com/razzant/ouroboros/releases/download/v7.0.0-rc.12/ouroboros-7.0.0-rc.12-1.red80.x86_64.rpm
[download-linux-appimage-x86_64]: https://github.com/razzant/ouroboros/releases/download/v7.0.0-rc.12/Ouroboros-7.0.0-rc.12-linux-x86_64.AppImage
[download-linux-x86_64]: https://github.com/razzant/ouroboros/releases/download/v7.0.0-rc.12/Ouroboros-7.0.0-rc.12-linux-x86_64.tar.gz
...
|---------|------|-------------|
| 7.0.0-rc.12 | 2026-09-05 | **Release candidate of 7.0.0 — hotfix of the rc.11 tag CI.** The rc.11 tag's 3-OS full-test was red on Windows and macOS only (the exact-SHA battery on the Linux release host was green): the live E2E stand's 0600 settings write used `os.fchmod` (absent on Windows) and its orphan scan read `/proc` (Linux only) — the write keeps fchmod where it exists and falls back to chmod, and without procfs a lane records a typed `orphan_scan=unavailable:no_procfs` fact instead of a passed check; the evolution campaign writer treated a campaign file that exists but cannot be read as no campaign and let a stale write win — present-but-unreadable now refuses the write; the network-password resolution order is a pure function (`resolve_network_password`) that the wrapper calls with the environment and the settings loader, so the resolution pin can no longer be reached by a leaked environment writer or a leaked patch on a shared test worker. |
| 7.0.0-rc.11 | 2026-09-05 | **Release candidate of 7.0.0 — the v7 follow-up sprint, phase F3 (health and economics) plus the second upstream absorption.** The frozen upstream `ouroboros` line up to db6d7cf8 is merged in (PR #609 net-resilience: interactive transport-wait episodes, a bounded paid repeat after a typed transport death, a shutdown-aware supervisor; PR #614 update letter: Ouroboros's own note about a pending official update on the Updates panel and in its runtime context), every upstream change relocated into v7's owning leaf. F3 lanes: review economics (span-only release carriers and prefix-duplicated governance docs leave the triad pack, the advisory transcript-cap overrun is a typed outcome, an offline pack meter under devtools/), the 7.0 deferral register re-dispositioned with a structural prose-id lint, the advisory mandatory-read corpus bound, the evolution restart-verify marker written even when auto-restart is off, the live E2E stand under devtools/e2e_live (SM1/SW1/SK1 over K staggered isolated servers, keyless stub mode, a run-wide budget cap, a detached clean seed, a confirmed-absorb gate for self-modification), and the workspace write-shape guard reading Windows path spellings correctly. |
