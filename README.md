...
[![Version 6.103.16](https://img.shields.io/badge/version-6.103.16-green.svg)](VERSION)
...
|---------|------|-------------|
| 6.103.16 | 2026-08-19 | test(web_search): fix infinite MagicMock recursion in completed-event mock and disable openrouter/anthropic cascade in `patch_env` so `_web_search` cascade is deterministic. Replaces `MagicMock` with `SimpleNamespace` for `response.usage` (the previous `model_dump()` returned a fresh MagicMock on every call, hanging the test indefinitely until pytest-timeout fired at 30s); `patch_env` now also clears `OPENROUTER_API_KEY`/`ANTHROPIC_API_KEY` so the openrouter chat.completions mock path doesn't shadow the "no answer" cascade that `test_streaming_empty_text_engages_cascade` verifies. Closes ibl-streaming-test-recursion-bugs. |
| 6.103.15 | 2026-08-19 | pytest suite rot class-fix (closes ibl-28f3c68cfaae, ibl-0a1c8a8b432f, ibl-c094f2f90ec4): add `timeout = 30` default to `[tool.pytest.ini_options]`, short-circuit `ouroboros.usage_ledger._fsync_path` when `OUROBOROS_PYTEST_ACTIVE` is set, drop the redundant per-test `@pytest.mark.timeout(10, method="signal")` from `tests/test_web_search.py::test_streaming_direct_openai_cost_remains_nullable` (its job is now done by the global timeout + fsync short-circuit), document the fixture-hang misattribution history in `tests/conftest.py::_rebind_runtime_roots_between_tests`. Also surface two pre-existing infinite-recursion bugs in `test_streaming_no_progress_without_search_events` and `test_streaming_empty_text_engages_cascade` (MagicMock `model_dump()` self-loop in `_obj_to_plain`); the new 30s default bounds them as pytest-timeout failures instead of the prior batch-context hang, and they are filed as a new backlog item (ibl-streaming-test-recursion-bugs) for the next cycle. |
| 6.103.14 | 2026-08-19 | feat(prompts): add WORKER_CONTRACTS.md with self-repo mutation rules; closes ibl-rot-fix-1-stash-antipattern (prompts/WORKER_CONTRACTS.md: new file with rot_fix/* + claudexord-stale-lock templates and explicit prohibition on git stash / git-write-ops via run_command on /opt/ouroboros). |
| 6.103.13 | 2026-08-19 | feat(prompts): add WORKER_CONTRACTS.md with self-repo mutation rules; closes ibl-rot-fix-1-stash-antipattern (prompts/WORKER_CONTRACTS.md: new file with rot_fix/* + claudexord-stale-lock templates and explicit prohibition on git stash / git-write-ops via run_command on /opt/ouroboros). |
