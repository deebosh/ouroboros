...
|---------|------|-------------|
| 6.114.4 | 2026-09-03 | fix(usage_ledger): compact_ledger folds fully-settled cold roots into pre-summed kind=compacted summary rows and archives raw rows to gzipped monthly files (closes ibl-2e24f465212d). Validator now accepts kind=compacted as a terminal-from-null kind. |
| 6.114.3 | 2026-09-03 | fix(size-ratchet): a git ref/object-unavailable failure no longer disables the whole local validator via "validator unavailable" — the previous-manifest resolution falls through to bootstrap and tip-exactness still runs (closes ibl-67aa1f89cf1c) |
| 6.114.2 | 2026-09-02 | fix(reflection): python3 -m ouroboros.reflection now auto-populates MEMORY_ACTIONS_JSON and BACKLOG_CANDIDATES_JSON from stored task_reflections.jsonl entries via new build_reflection_from_logs read-side helper (closes ibl-e387a221109d) |
| 6.114.1 | 2026-09-02 | Managed update to upstream v6.114.0 (b9bcc2da): hop 5 of 5 — acceptance_dialogue/delivery_protocol extraction, shell argv disclose-not-refuse (#447 A5), literal_argv_notes, result_envelope, process_facts, output_export_policy; fork carries preserved (validate_shell_argv narrowed, _locked_with_fsync, protected-path auto-restore, uncommitted-delivery guard, bump_version). |
