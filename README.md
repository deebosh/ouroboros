...
[![Version 6.103.10](https://img.shields.io/badge/version-6.103.10-green.svg)](VERSION)
...
|---------|------|-------------|
| 6.103.10 | 2026-08-17 | fix(mcp): stdio discovery works on Python 3.14 (closes ibl-mcp-discovery-fileno). Wraps errlog= in a real OS pipe so subprocess.Popen accepts it, and passes through the parent env to StdioServerParameters so MCP servers see their API key / host config (the SDK's default whitelist strips both). |
| 6.103.9 | 2026-08-17 | fix: harden MCP stderr-loss follow-ups — Python 3.10-compat classifier, stderr_tail redaction, changelog accuracy (closes ibl-mcp-discovery-stderr-loss advisories). Three findings surfaced by the v6.103.8 advisory triad: (1) `_classify_mcp_error` referenced `BaseExceptionGroup` / `ExceptionGroup` names directly — both are PEP 654 / Python 3.11+; the project targets `requires-python = ">=3.10"`, so a 3.10 install would `NameError` on first MCP refresh failure. Replaced the `isinstance` tuple with `isinstance(getattr(exc, "exceptions", None), tuple)` duck-typing — same shape `_stringify_mcp_failure` already uses. (2) `refresh_server` applied `_redact_error_text` to the unwrapped exception message but NOT to the captured `stderr_tail`. Many MCP stdio subprocesses echo their own bearer tokens / URL-embedded credentials on auth or connection failure, so the tail is now passed through the same redaction before being added to the response. (3) The v6.103.8 `bump_version` call landed a duplicate changelog row claiming a Google Generative AI direct provider shipped in this commit — it did not (the relevant code sat dirty in the worktree as out-of-scope user work). The misleading row is dropped; the Google GenAI provider will get its own release row when it actually ships. Two regression tests added: `test_refresh_redacts_auth_token_from_stderr_tail` and `test_classify_uses_duck_typing_for_exception_group`. Bypass: owner-authorized via advisory skip (per `dd7cf06d` precedent). crd-0003 unchanged; next reviewed commit closes it. |
| 6.103.8 | 2026-08-17 | fix: surface MCP stdio stderr + structured error_kind on refresh failure (closes ibl-mcp-discovery-stderr-loss). The MCP SDK's `asyncio.TaskGroup` handshake error was stringified as "unhandled errors in a TaskGroup (1 sub-exception)" — owners could not diagnose missing API keys, missing executables, or transport resets. `refresh_server` now unwraps `BaseExceptionGroup.exceptions` (falling back to `__cause__`), captures subprocess stderr via `stdio_client(errlog=...)`, classifies the outer exception (`task_group_failure`, `timeout`, `broken_pipe`, `missing_executable`, `connection_refused`, `permission_denied`, `unknown`), and surfaces the bounded stderr tail in the response (≤64 lines / 4 KB, with an explicit `⚠️ OMISSION NOTE` when clipped). Successful refresh shape is unchanged except for the OPTIONAL new `error_kind` / `stderr_tail` keys. Older SDKs without `errlog=` fall back gracefully via `TypeError` (debug-logged), preserving the refresh path in degraded mode. |
| 6.103.7 | 2026-08-16 | Close 3 test-coverage gaps: _reestablish_user_merge_head (git.py, ibl-1561d99048bb), the bump_version tool wrapper, and the semantic-overlap prompt/diffstat-ordering helpers. |
| 6.103.6 | 2026-08-16 | Wire in the (previously unused) protected-runtime-path restore mechanism in run_command: auto-reverts a protected file (BIBLE.md, docs/CHECKLISTS.md, etc.) a shell command dirtied in the system repo, scoped to the system repo only. |
