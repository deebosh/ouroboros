...
test('provider actions use the shared status-first action row contract', () => {
    ...
    const rows = [...html.matchAll(/<div class="settings-action-row(?:"|\s)[\s\S]*?<\/div>/g)]
        .map(([row]) => row);
    // Nine provider probes (v7 added DeepSeek) plus the catalog action. The
    // Claude-runtime status/Repair panel is retired with its product surface
    // (the advisory pre-review runs on a configured routed model or agent
    // session now).
