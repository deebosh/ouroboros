# Ouroboros v6.103.24 — Architecture & Reference
# Ouroboros v6.103.24 — Architecture & Reference
...
server.py (Starlette+uvicorn) ← HTTP + WebSocket on configurable host:port (default localhost:8765; Docker/non-loopback supported via OUROBOROS_SERVER_HOST=0.0.0.0)
  ...
  │   ├── update_merge.py      ← Managed-update engine: exact-target 3-way plan (every clean/conflicting plan also carries `overlap_candidates` from `update_semantic_overlap.py`), direct clean fast-forward, reviewed assisted merge materialization, transaction, verified rollback/smoke, and boot recovery
  │   ├── update_merge_policy.py ← Presentation-only doc/code/hot conflict labels plus the semantic-overlap advisory note appended to the assisted resolver's objective; every conflict uses the same reviewed assisted path
  │   ├── update_semantic_overlap.py ← (v6.103.0) Purely additive advisory layer: cheap pure-git `compute_overlap_candidates` (files touched by BOTH local and upstream history since their merge-base — catches non-overlapping-line duplicate fixes git's 3-way merge cannot see) and fail-soft, bounded, LIGHT-model `detect_semantic_overlap` (verdict per candidate file), plus a durable single-slot cache keyed on the exact base/target SHA pair. Never blocks or changes what `apply_managed_merge_update` does — advisory only
  │   └── update_watch.py      ← (v6.103.0) Read-only check-and-notify daemon thread (owner opt-in, off by default): periodically plans the managed update and, on a new target SHA, sends ONE de-duped chat notification naming the semantic-overlap flags if any. Never calls apply or touches the writer fence — auto-apply is impossible from this path by construction (BIBLE P0)
  │
  └── ouroboros/               ← Agent core (runs inside worker processes)
      ...
      ├── update_channels.py   ← Closed Stable/QA/Development mapping, update-network defaults, and the owner-facing autocheck toggle/interval + semantic-overlap timeout getters (all clamped, hot-reloadable, no restart)
      ...
      ├── projects_registry.py ← Multi-project registry: durable `data/state/projects.json` with immutable id/chat identity, 80-character display names, working-folder facts, `active|deleting|tombstoned` lifecycle, routing fence/generation, and visible revision; deletion preserves bindings/history/folder/memory and an id can never resurrect. `resolve_external_session_chat_id` (v6.102.0, session isolation by identity) auto-provisions a fileless Project (`origin="external_session"`) keyed by a deterministic hash of `(source, external_chat_id)`, so each distinct external conversation (two different Telegram chats, or two senders through the same skill) threads into its own room instead of collapsing into the shared main chat; the project's own chat_id stays the ordinary hash-derived positive value (never the raw external id — Telegram group/supergroup ids are negative and collide with the reserved A2A range, `contracts/chat_id_policy.py`), and a deleted session falls back to the shared main chat rather than being resurrected or losing the message. Wired into `supervisor/message_bus.py::enqueue_local_message` at ingress, before every downstream routing/history/WS-isolation consumer (`server.py::_route_owner_message`, `_reserved_project_for_chat`) ever sees the chat_id — so none of that already-hardened routing code needed to change.
...
The stdio transport pipes its subprocess's stderr through `stdio_client(errlog=...)` into a per-call `io.StringIO()` attached to the transport context. `_stringify_mcp_failure` walks `BaseExceptionGroup.exceptions` (falling back to `__cause__`) and emits one `f"{type(sub).__name__}: {sub}"` per sub-exception, plus a bounded tail of the captured stderr (last 64 lines OR 4 KB, whichever first, with an explicit `⚠️ OMISSION NOTE` when the bound clips). `_classify_mcp_error` maps the outer exception to a stable string kind — `task_group_failure`, `timeout`, `broken_pipe`, `missing_executable`, `connection_refused`, `permission_denied`, `unknown` — stored on the runtime as `last_error_kind` and surfaced on the refresh response as `error_kind`. The MCP SDK raises its handshake errors inside an `asyncio.TaskGroup`; without unwrapping, owners only ever see the opaque "unhandled errors in a TaskGroup (1 sub-exception)" string, which is structurally un-diagnosable. Older SDKs without `errlog=` support fall back to the SDK's default stderr path with `TypeError` caught and logged at debug, preserving the refresh path in degraded mode without `stderr_tail`.
...
| Managed update gateway ABI — the empty preflight request, exact channel-bound `UpdateMergePlan` (v6.103.0 adds `overlap_candidates` — pure-git semantic-overlap candidates computed on every clean/conflicting plan — and, once preflight has run the advisory model check, `semantic_overlap` plus `recommended_strategy_downgraded_reason`; all additive-optional, never change what apply does), pinned apply request (`strategy`, base/target SHAs, recovery confirmation), typed success/error variants, and `update_status_ready` WS notice that refreshes the boot-time cache in the UI. | `ouroboros/gateway/contracts.py`, `web/modules/api_types.js` | `tests/test_gateway_parity.py` pins every field and message type in both mirrors; `tests/test_update_apply_routing.py` drives pin, strategy, recovery-confirmation, and response routing; `tests/test_update_semantic_overlap.py` and `tests/test_update_watch.py` pin the advisory layer and watcher guard conditions. |
