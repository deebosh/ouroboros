...
margin after admission dispatches and may be cut by the deadline (cost at
most one floor-priced wave, session rows $0 API; outcome DEGRADED, never a
free skip). An
...
- `delegate_wait` is an event-only model sleep. Renew bounded transport windows in
  ...
  never change its own next answer. Polling writes nothing — except the usage
  ledger's own torn-tail quarantine after a SINGLE crash mid-append, which every
  reader performs identically (an ABSENT ledger file — `FileNotFoundError` —
  answers known-zero spend without the reader, and any other state goes through
  the reader; a crash inside the repair itself — a torn quarantine sink — is a
  known residual tracked as issue #27). Vendor-internal descendants stay opaque.
