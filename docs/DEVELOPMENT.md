...
The panel seam rechecks the launch floor and cancellation before the wallet
claim; deadline enforcement of a RUNNING panel belongs to the R23 clamps. An
...
- `delegate_wait` is an event-only model sleep. Renew bounded transport windows in
  ...
  ledger's own torn-tail quarantine after a crash mid-append, which every reader
  of that ledger performs identically. Vendor-internal descendants stay opaque.
