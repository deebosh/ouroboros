...
The paid claim itself rechecks the launch floor and cancellation immediately
before the wallet row — the launch rule's second and last evaluation per
panel, after loop admission — and a claim-side floor refusal takes the loop
gate's own typed skip (no reviewer run is recorded); deadline enforcement of
a RUNNING panel belongs to the R23 clamps. An
...
- `delegate_wait` is an event-only model sleep. Renew bounded transport windows in
  ...
  reader performs identically (an absent ledger answers known-zero spend without
