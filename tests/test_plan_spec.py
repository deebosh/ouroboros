...
def test_resolve_evidence_and_constitutional_never_raise_on_hostile_locators(tmp_path):
    ...
    # Python's `Path.resolve(strict=False)` stopped raising RuntimeError on a
    # symlink loop on some newer versions (3.14 here) — it silently returns the
    # unresolved path instead, so the loop is caught downstream as a generic
    # I/O failure rather than classified as symlink_loop. Either way the hostile
    # locator is correctly omitted (never attached, checked below).
    assert reasons["loop1"] in {"symlink_loop", "unreadable"}
