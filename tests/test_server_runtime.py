...
def test_a_fresh_local_first_install_still_authors_safety_light(tmp_path, monkeypatch):
    ...
    # ...and the pre-onboarding normalizers implement at least that decision. The
    # launcher has since gone further and persists nothing at all, which satisfies the
    # fresh-install rule by construction rather than by a carve-out; the server still
    # carries the guarded form (behavioural coverage: tests/test_onboarding_host.py).
