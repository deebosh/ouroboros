...
def test_migration_table_is_valid_and_uses_only_spec_approved_pending_owners():
    ...
    # Every row is a relocation inside the test tree: a moved test, stub or argv builder is owned by
    # the sibling module that now hosts it. No parent re-export appears because no parent keeps a
    # reference to what its siblings took, so every row's facade cell is "-".
    ...
    registry_extraction_no_facade_rows.update(w5_test_split_rows)
