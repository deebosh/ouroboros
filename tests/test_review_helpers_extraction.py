...
def test_review_helpers_extraction_size_bounds_have_meaningful_headroom():
    ...
    # +~9 over the upstream 850 target: the fork's oversized-function gate
    # (ibl-oversized-function-gate-bypass) re-exports 3 primitives from the new
    # ouroboros.tools.review_oversized leaf; the bodies moved out, the compat
    # re-export block stays here.
    assert counts["ouroboros.tools.review_helpers"] <= 900
