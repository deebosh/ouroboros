...
def test_headless_extraction_size_bounds_have_meaningful_headroom():
    ...
    # ouroboros.headless additionally carries the fork's razzant/ouroboros#139
    # prune_task_results GC alongside the upstream prune_* family (~90 lines over
    # the v7 leaf target; tracked debt ibl-local-56334da38429). Still far under
    # the 1600-line FUNCTION_DEBT module gate.
    assert counts["ouroboros.headless"] <= 1100
    assert all(
        count <= 1000 for name, count in counts.items() if name != "ouroboros.headless"
    )
