...
def test_outer_paid_stage_cannot_swallow_typed_unknown(tmp_path, monkeypatch, stage):
    ...
    # `_update_improvement_backlog` threads repo_dir= into append_backlog_items
    # (fork ibl-74bd59bab040 phantom-referent tagging), so the stub must accept kwargs.
    monkeypatch.setattr(improvement_backlog, "append_backlog_items", lambda *a, **k: 1)
