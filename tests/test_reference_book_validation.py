...
# NOTE (fork): upstream's `test_docs_sync_reads_full_chapters_and_never_loses_residue_between_files`
# monkeypatches `tests.test_docs_sync`'s `_read` to route through `load_reference_book`. This fork's
# `test_docs_sync.py` has not adopted that integration (it reads `docs/ARCHITECTURE.md` etc. directly),
# so the test is dropped rather than carried in a state that can never pass — see
