...
def test_active_chat_activity_contract_mirrors_direct_turn_shape():
    ...
    # get_type_hints (not raw __annotations__) resolves each field's ForwardRef to
    # its actual type object: with `from __future__ import annotations`, two
    # independently-declared TypedDicts with identical field shapes carry
    # ForwardRefs whose `owner` differs by class on newer Python (3.14+), which
    # makes raw __annotations__ dicts compare unequal despite an identical shape.
    assert typing.get_type_hints(ActiveChatActivity) == typing.get_type_hints(ActiveDirectTurn)
