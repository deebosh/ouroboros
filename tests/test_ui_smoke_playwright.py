...
def test_ui_smoke_agents_panel_list_editor(direct_server_with_data):
    """Settings → Agents: three compact subagent cards fit a laptop-height body; Add reveals
    the appended card with the caret in it and no error; only Save turns the empty route
    into a section-level line plus a tinted, self-naming card; Review lanes' Add lives in the
    group head and reveals its new row (docs/DESIGN.md "List editors")."""
    pytest.importorskip("playwright.sync_api", reason="Playwright is not installed")
    from playwright.sync_api import Error as PlaywrightError
    from playwright.sync_api import sync_playwright
