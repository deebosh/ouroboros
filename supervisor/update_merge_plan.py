...
def plan_managed_update_merge(
    ...
    # Advisory semantic-overlap pre-check (fork ibl feature, pairs with
    # supervisor.update_merge_policy.semantic_overlap_note): pure git, cheap,
    # fail-soft. Threaded into every ACTIONABLE plan (clean / assisted /
    # conflict) — never the unavailable/unknown/current bail-outs. Consumers
    # (update_watch, gateway.control) degrade to {} when it is absent.
