    ...
    *,
    ...
    try:
        ...
        if image_data:
            ...
            if not uploads:
                # ibl-162952d44079 / ibl-04a5239e3573: non-Web transports (Telegram
                # bridge, the legacy single-image base64 seam) carry image_data
                # instead of chat_attachment_uploads. Materialise the bytes to a
                # private staging file so the SHARED substrate reads them, instead
                # of leaving them inline-only (the agent could not read an inline
                # image via read_file(root='artifact_store', path='attachments/…')).
                # 50 MiB cap mirrors the upload guard used everywhere else. Failure
                # to persist leaves the inline image_base64 in place as a degraded
                # fallback — the same behaviour the rest of the substrate keeps.
                import base64
                import os
                ...
                try:
                    ...
                    if raw and len(raw) <= 50 * 1024 * 1024:
                        ...
                        uploads_dir = _pool().DRIVE_ROOT / "uploads"
